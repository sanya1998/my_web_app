...
117) Написать скрипты для измерения скорости ответов ручек, проверить без и с ORJSONResponse
...
123) Почитать про строку scope в форме с username и password  (users:read или users:write)
...
126) При включенном vpn не дернулась ручка локально
...
130) pre-commit добавить то, что алембиком создана последняя миграция и больше не было изменений