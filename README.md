...
28) сейчас не открывается http://0.0.0.0:8001/redoc. Можно сделать кастомный, будет работать. Но нужно проверить, заработает ли сам после обновления fastapi
29) if TYPE_CHECKING: везде, где импортируемое используется только для аннотации
30) Динамическе навешивание декораторв позволяет удобно дебажить, пропуская декораторы (см BaseESClient). Это можно применить для exceptions
