...
49) В декортаор роутера вставлять dependencies=, которые не используются внутри ендпоинта
50) pyright, mypy поробовать (предварительно надо навести порядок в базовом классе BaseRepo)
51) вместо того, чтобы писать алгоритм создания конфигурации, добавить в git диеркторию .idea, если это безопасно
