...
### Инструкция по деплою веб-приложения. (TODO: сравнить с translated_telegram)
...
- записать переменные окружения в новый файл: nano my_web_app/envs/dev.env
- установить make: `apt install make`
- запустить подготовленную сборку `make build-dev-app`
- перейти по http://109.73.207.254:8000/docs (TODO: разобраться)
...
28) Шаблон readme https://gist.github.com/bzvyagintsev/0c4adf4403d4261808d75f9576c814c2#технологии
29) сейчас не открывается http://0.0.0.0:8001/redoc. Можно сделать кастомный, будет работать. Но нужно проверить, заработает ли сам после обновления fastapi
30) if TYPE_CHECKING: везде, где импортируемое используется только для аннотации
31) Динамическе навешивание декораторв позволяет удобно дебажить, пропуская декораторы (см BaseESClient). Это можно применить для exceptions
...
2) можно ли как-то индексировать таблицу в бд, чтоб быстро сортировалось по полю? время выполнения запроса. просто добавить индексы (см news_migrator и translated_telegram)
...
3) Попробовать всю мощь app=StaticFiles
...
10) id: uuid.UUID
11) В приветственных логах api написать ссылку на сваггер, чтобы не делать лишних переходов. можно еще так `docs_url="/",`

### Elasticsearch
1) Безопасность. Пароль главный или пароль для подключения из сервисов. SSL
2) См файл es/es_идеи.txt
