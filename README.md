...
117) Написать скрипты для измерения скорости ответов ручек, проверить без и с JSONResponse
118) Tags из роутеров в Enum
119) в документации fastapi часто вместо model_dump используется dict. Уточнить как правильнее, на данный момент
120) По OAuth2 "Поток пароля" требует обязательного наличия в форме строго именованных полей `username` (не `email`) и `password`.
121) Рассмотреть возможность передавать ошибки в responses. Выбрать оптимальный вариант. Вероятно, в свагере появятся