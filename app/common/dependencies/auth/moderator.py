...
async def get_current_moderator_user(user: Annotated[UserReadSchema, Depends(get_current_user)]):
    if RolesEnum.MODERATOR not in user.roles:
        raise ForbiddenApiError
    return user


CurrentModeratorUserDep = Annotated[UserReadSchema, Depends(get_current_moderator_user)]
